...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{`len("Hello World")`, 11},
		{`let a = [1, 2, 3]; len(a);`, 3},
		{`let a = [3, 2, 1]; first(a)`, 3},
		{`let a = [3, 2, 1]; last(a)`, 1},
		{`let a = [true, "World", 1]; first(a)`, true},
		{`let a = ["Hello", "World", 1]; first(a)`, "Hello"},
		{`let a = [true, "World", 1]; last(a)`, 1},
		...
		{`first("")`, &object.Error{Message: "argument to `first` must be ARRAY, got STRING"}},
	...
	for _, tt := range tests {
		...
		default:
			t.Fatalf("Unknown expected type. got=%T", expected)
		}
	}
}
...
func testStringObject(t *testing.T, obj object.Object, expected string) bool {
	...
	if result.Value != expected {
		t.Errorf("String has wrong value. got=%q", result.Value)
		return false
	}
